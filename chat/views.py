...
def index(request):
    if request.method == "GET":
        ...
        longPollingSecondsRemaining = 5.0
        SLEEP_INTERVAL = 0.5
        while len(messages) < 1 and longPollingSecondsRemaining > 0.0:
          sleep(SLEEP_INTERVAL)
          longPollingSecondsRemaining -= SLEEP_INTERVAL
          messages = Message.objects.all().filter(id__gt=lastSeenId)

        POPMessages = map(lambda m: {'id': m.id, 'sender': m.sender, 'body': m.body}, messages)
